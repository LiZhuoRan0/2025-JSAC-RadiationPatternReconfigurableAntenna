function [Best_RadiationPattern,...
          ...
          Best_Cmber,...
          Sele_Cmber] = Gen_RadiationMode_Cmber_OMP_NoJammer(Para)
...
gain = mean(abs(Para.gain_true), 2);% Para.L_C x 1
...
%% -    2.1 Select Radiation Pattern
    P_B_C = zeros(length(Para.varphi_C_rx_ture), 1);
    for i = 1:length(Para.varphi_C_rx_ture)
        P_B_C(i) = abs(Best_Cmber' * Gen_SteeringVec(Para.varphi_C_rx_ture(i),...
                 Para.theta_C_rx_ture(i), ...
    ...
    Numerator = sum(repmat(gain.^2 .* P_B_C, 1, Para.N_h_C) .* (Para.C_alg(:,index_C).^2).', 1);
...
%% -    2.2 Design Combiner
    A = zeros(Para.N_ant_U, Para.N_ant_U);
    for i = 1:length(Para.varphi_C_rx_ture)
        A = A + gain(i)^2 * Para.C_alg(Best_RadiationPattern,index_C(i)).^2 *...
            Gen_SteeringVec(Para.varphi_C_rx_ture(i),...
                 Para.theta_C_rx_ture(i), ...
...
gain = mean(abs(Para.gain)/10^(Para.P_C/20), 2);% Para.L_C(est.) x 1
index_C = (Para.varphi_C_rx+90)*181 + Para.theta_C_rx+91;
% index_J = (Para.varphi_J+90)*181 + Para.theta_J+91;
%% 2. Alternative Optimization
while SIR_1 - SIR_2 > epsilon
%% -    2.1 Select Radiation Pattern
    P_B_C = zeros(length(Para.varphi_C_rx), 1);
    for i = 1:length(Para.varphi_C_rx)
        P_B_C(i) = abs(Sele_Cmber' * Gen_SteeringVec(Para.varphi_C_rx(i),...
                 Para.theta_C_rx(i), ...
                 Para.N_ant_ele_U, Para.N_ant_azi_U))^2;
    end
...
%% -    2.2 Design Combiner
    A = zeros(Para.N_ant_U, Para.N_ant_U);
    for i = 1:length(Para.varphi_C_rx)
        A = A + gain(i)^2 * Para.C_alg(Sele_RadiationPattern,index_C(i)).^2 *...
            Gen_SteeringVec(Para.varphi_C_rx(i),...
                 Para.theta_C_rx(i), ...
                 Para.N_ant_ele_U, Para.N_ant_azi_U) * ...
            Gen_SteeringVec(Para.varphi_C_rx(i),...
                 Para.theta_C_rx(i), ...
