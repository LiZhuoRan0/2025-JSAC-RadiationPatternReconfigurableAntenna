...
SavePath = sprintf('../Data/Simu/NoJammer_%s_%s_%s_N_%d_Antenna_%d.mat', ...
...
for n_sit = 1:LenSituation
    ...
    for n_it = 1:N_it
        ...
        Y_C2U = Gen_RcvSig(Para, 'Ctrler2UAV_NoJammer');
        if OMP_MUSIC == 0
        ...
        %% 2. Choosing Radiation Pattern
        if OMP_MUSIC == 0
            [Best_RadiationPattern, Sele_RadiationPattern, Best_Cmber, Sele_Cmber] = Gen_RadiationMode_Cmber_OMP_NoJammer(Para);
        else
            [Best_RadiationPattern, Sele_RadiationPattern, Best_Cmber, Sele_Cmber] = Gen_RadiationMode_Cmber_MUSIC_NoJammer(Para);
        end
        
        %% 3. Evaluate
        % Sensing Error
        Error_sensing = Eval_Sensing_NoJammer(Para);
        
        % Spectrum Efficiency
        [SE_tmp_up, SE_tmp_dn] = Eval_SE_NoJammer(Para, Best_RadiationPattern, Sele_RadiationPattern, Best_Cmber, Sele_Cmber);
