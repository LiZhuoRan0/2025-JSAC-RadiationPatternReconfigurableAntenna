...
methods
%% 
function obj = ParaClass(P_C, P_J, HPBW, N, N_rx_ele, N_rx_azi, threshold, LoS, Low)
    obj.LoS = LoS;
    obj.Low = Low;
    % independent 
...
%     obj.G_J                 = 10;% dBi
    ...
    % others
    obj.HPBW                = HPBW;
    filename_real = sprintf('../../Data/RadiationPattern/HPBW_%d/RadiationPattern_%d.mat', obj.HPBW, obj.N_p_azi);
    if Low == 1
        filename_alg = filename_real;
    else
        filename_alg = sprintf('../../Data/RadiationPattern/HPBW_%d/RadiationPattern_%d_%ddB.mat', obj.HPBW, obj.N_p_azi, -threshold);
    end
    ...
    obj.C_alg = sqrt(load(filename_alg).Result);
...
function obj = Gen_Pattern(obj)
    % obj.N_h_J x 1
    index_J = (round(obj.varphi_J_ture)+90)*obj.N_a_ele + round(obj.theta_J_ture)+91;
    obj.Ptn_J = obj.C(:, index_J);

    % obj.N_h_C x obj.L_C
    index_C = (round(obj.varphi_C_rx_ture)+90)*obj.N_a_ele + round(obj.theta_C_rx_ture)+91;
...
function obj = Up_Gain(obj, n_RadiationPattern)
    ...
    if obj.LoS == 0% LoS
        obj.Gain_J2U = 3e8/obj.f_c/...
                      ...
                      round(obj.theta_J_ture) + 91 + (round(obj.varphi_J_ture) + 91 - 1)*181);
    else% NLoS
        obj.Gain_J2U = 3e8/obj.f_c/...
                      ((4*pi)^1.5 * obj.r_J_LoS * obj.r_J_NLoS)*...
                      sqrt(obj.RCS)*...
    ...
    if obj.LoS == 0% LoS
        obj.Gain_C2U(1) = 3e8/obj.f_c/...
                      (4*pi*obj.r_C_LoS)*...
                      obj.C(n_RadiationPattern,...
                      round(obj.theta_C_rx_ture(1)) + 91 + (round(obj.varphi_C_rx_ture(1)) + 91 - 1)*181);
    else% NLoS
        obj.Gain_C2U(1) = 3e8/obj.f_c/...
                      ((4*pi)^1.5 * obj.r_C_LoS * obj.r_C_NLoS)*...
                      obj.C(n_RadiationPattern,...
    ...
    obj.Gain_C2U(2) = 3e8/obj.f_c/...
                  ((4*pi)^1.5 * obj.r_C_LoS * obj.r_C_NLoS)*...
                  obj.C(n_RadiationPattern,...
                  round(obj.theta_C_rx_ture(2)) + 91 + (floor(obj.varphi_C_rx_ture(2)) + 91 - 1)*181)...
                  *sqrt(obj.RCS);
end
