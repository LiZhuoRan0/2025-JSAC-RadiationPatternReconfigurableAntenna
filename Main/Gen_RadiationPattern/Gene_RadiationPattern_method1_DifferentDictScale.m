...
DictScale = 0.5;
HPBW = 65;
N = 11; % Number of axis samples for both phi and theta axes
% Axis angles for phi and theta, from -90° to 90°
...
num_samples = length(phi_samples)*length(theta_samples); % 181*181 = 32761
...
for i = 1:N
    ...
    tmp = zeros(N, length(phi_samples)*length(phi_samples));
    for j = 1:N
        current_theta_line = theta_axis(j);
        
        % Generate gain pattern (linear scale)
        Gain = generate_gain_pattern(HPBW, current_phi_line, current_theta_line, phi_samples, theta_samples);
...
% normlization
Result = RadiationPattern_Normlization_DifferentDictScale(Result, N, DictScale);
filename = sprintf('../../Data/RadiationPattern/HPBW_%d/RadiationPattern_%d_Scale_%d.mat', HPBW, N, DictScale);
save(filename, 'Result')
%% Visualization to verify correctness
% 1. Visualize a single pattern from Result
pattern_idx = floor(N/2)*N + ceil(N/2);
pattern_2D = reshape(Result(pattern_idx,:), length(theta_samples), length(phi_samples));

% Convert pattern to dB for coloring
pattern_dB = 10*log10(pattern_2D);

[PHI_deg, THETA_deg] = meshgrid(phi_samples, theta_samples);
...
h = colorbar;              
h.Label.String = 'dBi';
clim([min(pattern_dB(:)), max(pattern_dB(:))]);
...
for k = 1:num_combinations
    temp_2D = reshape(Result(k,:), length(theta_samples), length(phi_samples));
...
arrow_size = 0.5;
quiver3(0,0,0, axis_scale_comb*1.3,0,0, 'Color','g','LineWidth',2,'MaxHeadSize',arrow_size);
text(axis_scale_comb*1.4,0,0,'X','FontSize',20,'FontWeight','Bold','Color','g');
...
title('');
xlabel('Angle Index');
ylabel('Pattren Index');
set(gca,'FontName','Times New Roman','FontSize',20);
%% Function: generate_gain_pattern
function Gain = generate_gain_pattern(HPBW, phi_line, theta_line, phi_samples, theta_samples)
    deg2rad_factor = pi/180;
