...
for i = 1:N
    ...
    for j = 1:N
        ...
        Gain = generate_gain_pattern(current_phi_line, current_theta_line, phi_samples, theta_samples);
...
toc
filename = sprintf('../../Data/RadiationPattern/RadiationPattern_%d.mat', N);
...
function Gain = generate_gain_pattern(phi_line, theta_line, phi_samples, theta_samples)
    deg2rad_factor = pi/180;
    ...
    A_EV0 = -min(12*((PHI_rel/(theta_3dB*deg2rad_factor)).^2), SLA_V);
    A_EH0 = -min(12*((THETA_rel/(phi_3dB*deg2rad_factor)).^2), A_m);
