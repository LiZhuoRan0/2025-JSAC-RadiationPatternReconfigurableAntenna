...
%% ===== 2. 选定 N=11 且 HPBW 在目标集合中的索引 =====
HPBW_target = [15 25 45 65];
idx_target  = find( N_all == 11  &  ismember(HPBW_all, HPBW_target) );

% 为了保持目标 HPBW 的顺序，重新按 HPBW_target 排序索引
idx_ordered = arrayfun(@(h) idx_target(HPBW_all(idx_target)==h), HPBW_target);

%% ===== 3.1 计算均值 & 标准差（先对路径均值，再对实验均值/方差） =====
...
for k = 1:numel(idx_ordered)
    ...
    % --- Independent ---
    tmp   = squeeze( AngleErr_ind(:,1,id) );      % 10000×2
    mu_ind(k) = mean(tmp);
    sd_ind(k) = std(tmp);
...
% 轴 & 注释
xticks(x);
xticklabels(string(HPBW_target));
xlabel('HPBW (°)');
ylabel('Angle Error (°)');
legend({'Indep. ±1σ','Indep. μ', ...
        ...
        'Location','northwest');
grid on;
title('Angle Error vs. HPBW  (N = 11)');

%% ===== 4.1 计算均值 & 标准差（先对路径均值，再对实验均值/方差） =====
mu_ind  = zeros(1,numel(idx_ordered));
sd_ind  = zeros(1,numel(idx_ordered));
mu_nind = zeros(1,numel(idx_ordered));
sd_nind = zeros(1,numel(idx_ordered));
...
for k = 1:numel(idx_ordered)
    ...
    % --- Independent ---
    tmp   = squeeze( AngleErr_ind(:,2,id) );      % 10000×2
    mu_ind(k) = mean(tmp);
    sd_ind(k) = std(tmp);
